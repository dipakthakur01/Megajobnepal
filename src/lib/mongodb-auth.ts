// MongoDB-based authentication service for MegaJobNepal
import { dbService, User } from './mongodb-real';
...
class MongoDBAuthService {
  ...
  async signIn(email: string, password: string): Promise<{ token: string; user: User } | { error: string }> {
    try {
      ...
      if (!user) {
        // Special handling for demo accounts - try to create if missing
        if (email.includes('.demo@megajobnepal.com')) {
          console.log(`🔧 Demo account ${email} not found, attempting to create...`);
          return { error: 'Demo account not found. Please run database setup first.' };
        }
      ...
      // Special handling for demo accounts - auto-verify if needed
      if (!user.is_verified && email.includes('.demo@megajobnepal.com')) {
        console.log(`🔧 Auto-verifying demo account: ${email}`);
      ...
      // Check if user is verified (skip for demo accounts)
      if (!user.is_verified && !email.includes('.demo@megajobnepal.com')) {
...
export const mongodbAuthService = new MongoDBAuthService();
...
import { Upload, Edit, Heart, Clock, CheckCircle, XCircle, User, FileText, Bell, Eye, Download, MapPin, Calendar as CalendarIcon, Briefcase, GraduationCap, Award, Plus, Trash2, AlertCircle, Map, BookmarkPlus } from 'lucide-react';
...
export function JobSeekerDashboard({ user, jobs, applications, onApplicationUpdate, onLogout }: JobSeekerDashboardProps) {
  ...
  const getRecommendedJobs = React.useMemo(() => {
    ...
    return jobs.filter(job => {
      ...
      const matchesJobType = job.employment_type && profile.preferredJobTypes.includes(job.employment_type);
      ...
      const matchesLocation = profile.preferredLocations.some(loc => {
        ...
        if (job.location && job.location.city) {
          return job.location.city.toLowerCase().includes(loc.toLowerCase());
        }
        // Fallback check for string-based location
                  ...
                  <div key={job.id} className="border rounded-lg p-4 hover:shadow-sm transition-shadow">
                    ...
                    <div className="flex items-center justify-between">
                      <Badge variant="outline" className="text-xs">
                        {job.employment_type ? job.employment_type.replace('_', ' ') : 'Not specified'}
                      </Badge>
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        ...
                        onSelect={(date) => setNewEducation({...newEducation, endDate: date})}
                        initialFocus
                      />
                    </PopoverContent>
                  </Popover>

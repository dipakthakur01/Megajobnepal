...
import companyLogo from 'figma:asset/58765e98d4cbaf5fa8ee50255793eaa2f2116f36.png';
import { 
  User as UserIcon, 
  Briefcase, 
  Building2, 
...
export function Header({ onNavigate, onLogin, onLogout, currentPage }: HeaderProps) {
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {currentUser.role !== 'admin' && (
                    <div className="relative">
                      <button
                        onMouseEnter={() => {
                          if (currentUser.type === 'jobseeker') setShowJobseekerDropdown(true);
                          if (currentUser.type === 'employer') setShowEmployerDropdown(true);
                        }}
                        onMouseLeave={() => {
                          if (currentUser.type === 'jobseeker') setShowJobseekerDropdown(false);
                          if (currentUser.type === 'employer') setShowEmployerDropdown(false);
                        }}
                        className="flex items-center space-x-2 px-3 py-2 text-sm text-gray-700 hover:text-blue-600 transition-colors"
                      >
                        <span>For {currentUser.type === 'jobseeker' ? 'Jobseekers' : 'Employers'}</span>
                        <ChevronDown className="w-4 h-4" />
                      </button>
                      
                      {/* Logged in user dropdown */}
                      {((currentUser.type === 'jobseeker' && showJobseekerDropdown) || (currentUser.type === 'employer' && showEmployerDropdown)) && (
                        <div 
                        ...
                        >
                          <div className="p-4">
                            <h3 className="font-semibold text-gray-900 mb-3">
                              {currentUser.type === 'jobseeker' ? 'Job Seeker Tools' : 'Employer Tools'}
                            </h3>
                            <div className="space-y-2">
                              {(currentUser.type === 'jobseeker' ? jobseekerMenuItems : employerMenuItems).map((item, index) => {

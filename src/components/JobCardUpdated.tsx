...
export const JobCard = React.memo(function JobCard({ job, onViewJob, tierColor, tierBgColor, onSaveJob, onLoginRequired, isSaved = false, isUserLoggedIn = false, onViewCompany }: JobCardProps) {
  ...
  return (
    <Card 
      className={`group hover:shadow-xl hover:shadow-${tierInfo.glow} hover:scale-[1.02] transition-all duration-500 cursor-pointer border-2 ${tierInfo.border} ${tierInfo.bg} relative overflow-hidden rounded-2xl h-[380px] flex flex-col backdrop-blur-sm hover:rotate-1 hover:-rotate-1`}
    ...
    >
      ...
      <button
        ...
        className={`absolute top-3 left-3 z-20 p-1.5 rounded-full transition-all duration-300 hover:scale-110 ${
          isBookmarked 
            ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' 
            : 'bg-white/80 text-gray-600 hover:bg-white hover:text-orange-500'
        } backdrop-blur-md`}
      >
        <Bookmark className={`w-3.5 h-3.5 ${isBookmarked ? 'fill-current' : ''} transition-all duration-200`} />
      </button>
      ...
      <CardContent className="p-5 flex flex-col h-full relative z-10">
        ...
        <div className="flex items-start space-x-3 mb-3 h-[70px] flex-shrink-0">
          <div className="relative flex-shrink-0 group/logo">
            <div className="absolute inset-0 bg-gradient-to-br from-orange-400/20 to-blue-400/20 rounded-xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <ImageWithFallback
              src={job.logo}
              alt={job.company}
              className={`w-14 h-14 rounded-xl object-cover border-2 border-white shadow-lg relative z-10 transition-transform duration-500 ${isHovered ? 'scale-110 rotate-3' : ''}`}
            />
            {job.featured && (

...
export const HomePage = React.memo(function HomePage({ jobs, onNavigate, onViewJob, filters, onFilterChange, onSaveJob, savedJobs = [], isUserLoggedIn = false, onLoginRequired }: HomePageProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [currentTestimonial, setCurrentTestimonial] = useState(0);
  const [showRecruitmentPopup, setShowRecruitmentPopup] = useState(false);
  const [showPricingInPopup, setShowPricingInPopup] = useState(false);
  const [isLoading, setIsLoading] = useState(true);

  // Initialize component and set loading state
  useEffect(() => {
    // Simulate loading time and then show popup
  ...
  const handleClosePopup = () => {
    ...
    setShowPricingInPopup(false);
  };

  const handleShowPricing = () => {
    setShowPricingInPopup(true);
  };
  ...
  const deduplicateJobsByCompany = (jobList: Job[]) => {
    ...
    for (const [companyName, companyJobs] of companyJobsMap) {
  ...
  const jobsByTier = useMemo(() => {
    ...
    for (const job of jobs) {
      if (job.tier === 'megajob') {
        ...
        premium.push(job);
      } else if (job.tier === 'prime') {
        prime.push(job);
      } else if (job.tier === 'latest' || job.tier === 'latest_job') {
        latest.push(job);
      } else if (job.tier === 'newspaper' || job.tier === 'newspaper_job') {
        newspaper.push(job);
      }
      
      // Also categorize based on source for latest jobs
      if (job.source === 'online' && !['megajob', 'premium', 'prime'].includes(job.tier as string)) {
    ...
    <div className="min-h-screen">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative">
                  ...
                  <Input placeholder="Location" className="pl-10 w-full sm:w-40" />
                </div>
                <div className="relative">
                  <Briefcase className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                  <Input placeholder="Category" className="pl-10 w-full sm:w-40" />
                </div>
        ...
        <div className="max-w-8xl mx-auto">
          ...
          <div>
            ...
            {jobsByTier.megajob.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 job-grid">
      ...
      {showRecruitmentPopup && (
        <RecruitmentPopup 
          ...
          showPricing={showPricingInPopup}

...
export function HeroImageManagement() {
  ...
  const saveImages = (imagesToSave: HeroImage[]) => {
    try {
      localStorage.setItem('megajob_hero_images', JSON.stringify(imagesToSave));
    } catch (error) {
  ...
  const saveSettings = (settingsToSave: HeroSettings) => {
    try {
      localStorage.setItem('megajob_hero_settings', JSON.stringify(settingsToSave));
  ...
  const updateImage = (imageId: string, updates: Partial<HeroImage>) => {
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => setEditingImage(prev => prev ? { ...prev, isActive: checked } : null)}
                />
              </div>

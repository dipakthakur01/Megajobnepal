...
export function CompanyManagementEnhanced({ companies, jobs, onCompanyUpdate }: CompanyManagementEnhancedProps) {
  ...
  const handleCreateCompany = () => {
    ...
    const company = {
      ...
      logo: newCompany.logoUrl || (newCompany.profileImage ? URL.createObjectURL(newCompany.profileImage) : 'https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=100&h=100&fit=crop'),
      status: 'active',
    ...
    const updatedCompanies = companies.map(company =>
      company.id === selectedCompany.id
        ? {
            ...
            values: Array.isArray(selectedCompany.values)
              ? selectedCompany.values
              : selectedCompany.values.split('\n').filter((v: string) => v.trim()),

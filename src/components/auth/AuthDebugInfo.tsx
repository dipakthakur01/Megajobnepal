...
export function AuthDebugInfo() {
  ...
  // Check MongoDB configuration on mount
  useEffect(() => {
    const configured = isSupabaseConfigured(); // Keep for backwards compatibility
    console.log('MongoDB Auth configured:', !configured); // Inverted logic since false means MongoDB
    if (!configured) {
      setExpanded(true);
    }
  }, []);
  ...
  const checkHealth = async () => {
    setLoading(true);
    try {
      // Check MongoDB health by testing localStorage access
          ...
          <div className="space-y-2 text-xs text-gray-700">
            {!configured && (
              <div className="space-y-1">
                <div>• <strong>Database ready:</strong> MongoDB with browser storage is properly configured</div>
